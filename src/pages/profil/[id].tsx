...
const Home: NextPage = () => {
    ...
    const {data:profile, isLoading: profileLoading} = trpc.useQuery(["participants.getOne", parsedId]);
    ...
    return (
        <>
            <Head>
                <title>{profile ? `${profile.name} - Défi Bacon` : "Défi Bacon"}</title>
                <meta name="description" content={profile ? `${profile.name} a couru ${profileDistance ?? 0} KM pour la cause.` : "Défi Bacon - Courir pour la cause."} />
            ...
            <div className={"container mx-auto md:py-20 md:px-16 py-10 px-8 flex flex-col"}>
                <div className={"mb-5 md:mb-10 self-start flex flex-row space-x-2"}>
                    <button onClick={()=>router.push("/app")} aria-label="Retour" className={"hover:opacity-70 transition-opacity"}>
                        <ArrowLeftIcon className={"w-8 md:w-12 text-black cursor-pointer"}/>
                    </button>
                </div>
                {profileLoading && (
                    <div className={"text-dark text-2xl font-poppins"}>Chargement du profil...</div>
                )}
                {!profileLoading && !profile && (
                    <div className={"text-dark text-2xl font-poppins"}>Participant introuvable.</div>
                )}
                {profile && (<>
                <div className={"md:text-8xl text-5xl mb-10 text-dark font-poppins uppercase font-bold"}>{profile.name}</div>
                <div className={"md:text-5xl text-4xl font-rubik text-dark"}>a couru {profileDistance ?? 0} KM pour la cause... {`c'est`}  {percent.toFixed(2)} % de {`l'objectif.`}</div>
