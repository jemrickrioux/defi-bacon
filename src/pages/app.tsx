import type { NextPage } from "next";
import Head from "next/head";
import { trpc } from "../utils/trpc";
import Form from "../components/Form";
import ProgressBar from "../components/ProgressBar";
import Link from "next/link";
import { useEffect, useCallback, useState } from "react";
import { HomeIcon, PlusIcon } from "@heroicons/react/solid";
import { useRouter } from "next/router";

const App: NextPage = () => {
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);
  const formatter = Intl.DateTimeFormat("fr-CA", {
    dateStyle: "medium",
    timeZone: "America/Montreal",
  });

  const toggleModal = useCallback(() => {
    setIsOpen((prev) => !prev);
  }, []);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && isOpen) {
        setIsOpen(false);
      }
    };
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [isOpen]);
  const { data: current } = trpc.useQuery(["goal.current"]);
  const { mutateAsync: mutate } = trpc.useMutation("participations.addOne");
  const {
    data: p,
    isLoading,
    refetch: refetchAll,
  } = trpc.useQuery(["participations.getAll"]);
  const {
    data: l,
    isLoading: lbLoading,
    refetch: refetchLeaderboard,
  } = trpc.useQuery(["participants.leaderboard"]);
  const { data: total, refetch: refetchDistance } = trpc.useQuery([
    "participations.totalDistance",
  ]);
  const { data: goal } = trpc.useQuery(["goal.totalDistance"]);
  const refresh = async () => {
    await Promise.all([refetchAll(), refetchDistance(), refetchLeaderboard()]);
  };
  return (
    <>
      <Head>
        <title>Résultats - Défi Bacon</title>
        <meta name="description" content="Classement et dernières courses du Défi Bacon. Courir pour la cause en mémoire de Maxime Papillon." />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      {isOpen && (
        <div
          className={
            "inset-0 md:bg-hero-i-like-food-100 absolute z-2 bg-dark bg-opacity-80 flex justify-center"
          }
          onClick={(e) => {
            if (e.target === e.currentTarget) setIsOpen(false);
          }}
        >
          <Form
            mutate={mutate}
            refresh={refresh}
            toggleModal={toggleModal}
          />
        </div>
      )}
      <div className={" z-4 container mx-auto md:mt-10 md:w-3/4 flex flex-col"}>
        <div
          className={`${
            isOpen ? "hidden" : ""
          } flex h-max flex-col bg-dark  md:rounded-lg md:px-24 px-10 md:py-20 py-10 md:my-5`}
        >
          <div className={"md:mb-10 mb-5 self-start flex flex-row space-x-2"}>
            <button onClick={() => router.push("/")} aria-label="Accueil" className={"hover:opacity-70 transition-opacity"}>
              <HomeIcon className={"w-8 md:w-12 text-white mb-5 cursor-pointer"} />
            </button>
          </div>
          <div
            className={
              "flex md:flex-row w-full md:justify-between justify-between"
            }
          >
            <div
              className={
                "md:text-6xl text-2xl mb-14 text-white uppercase font-bold font-poppins"
              }
            >
              Résultats
            </div>
            <button
              onClick={toggleModal}
              className={
                "cursor-pointer md:text-xl text-sm mb-14 self-center hover:shadow hover:text-white hover:bg-primary md:self-center w-max h-max text-dark uppercase bg-secondary md:py-4 py-2 px-4 rounded-lg font-bold font-poppins flex items-center transition-colors"
              }
            >
              <PlusIcon className={"w-6"} />{" "}
              <span className={"ml-2"}>Course</span>
            </button>
          </div>
          <ProgressBar
            current={total ? total : 0}
            goal={current ? current / 10 : 0}
            type={"distance"}
          />
          <div className={`border-b-2 -mt-5 mb-14 `}></div>
          <div
            className={
              "md:text-6xl text-4xl md:mb-14 mb-8 text-white uppercase font-bold font-poppins"
            }
          >
            Top 10
          </div>
          {lbLoading && (
            <div className={"text-white text-2xl font-poppins"}>Chargement...</div>
          )}
          {!lbLoading && (!l || l.length === 0) && (
            <div className={"text-white text-2xl font-poppins opacity-60"}>Aucun participant pour le moment.</div>
          )}
          {l?.slice(0, 10).map((el: any, index: number) => {
            return (
              <div className="my-1" key={el.id}>
                <div className="flex flex-row space-x-2 items-center md:py-6 py-4 md:px-14 px-6 justify-between bg-primary rounded-2xl shadow">
                  <div
                    className={
                      "flex font-bold text-white text-4xl font-bold text-5xl"
                    }
                  >
                    {index + 1}
                    <div className={"flex flex-col ml-6 justify-center"}>
                      <div
                        className={
                          "md:text-4xl text-2xl text-white font-poppins underline hover:text-dark hover:animate-pulse underline-offset-1 "
                        }
                      >
                        <Link href={`/profil/${el.participant.id}`}>
                          {el.participant.name} <span aria-hidden="true">&rsaquo;</span>
                        </Link>
                      </div>
                    </div>
                  </div>

                  <div
                    className={
                      "md:text-5xl text-xl text-white w-max flex md:self-center self-start"
                    }
                  >
                    {el.distance.toFixed(2)}KM
                  </div>
                </div>
              </div>
            );
          })}

          <div className={`border-b-2 my-14 `}></div>
          <div
            className={
              "md:text-6xl text-4xl md:mb-14 mb-8 text-white uppercase font-bold font-poppins"
            }
          >
            Dernières courses
          </div>
          {isLoading && (
            <div className={"text-white text-2xl font-poppins"}>Chargement...</div>
          )}
          {!isLoading && (!p || p.length === 0) && (
            <div className={"text-white text-2xl font-poppins opacity-60"}>Aucune course enregistrée.</div>
          )}
          {p?.slice(0, 20).map((el: any) => {
            return (
              <div className="my-1" key={el.id}>
                <div className="flex flex-row space-x-2 items-center md:py-6 py-4 md:px-14 px-6 justify-between bg-primary rounded-2xl shadow">
                  <div className={"flex flex-col"}>
                    <div
                      className={
                        "md:text-4xl text-2xl text-white font-poppins underline hover:text-dark hover:animate-pulse underline-offset-1"
                      }
                    >
                      <Link href={`/profil/${el.participant.id}`}>
                        {el.participant.name} <span aria-hidden="true">&rsaquo;</span>
                      </Link>
                    </div>
                    <div
                      className={"md:text-2xl text-lg text-dark mt-2"}
                    >
                      {formatter.format(el.date)}
                    </div>
                  </div>
                  <div
                    className={
                      "md:text-5xl text-xl text-white w-max flex md:self-center self-start"
                    }
                  >
                    {el.distance}KM
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </>
  );
};

export default App;
